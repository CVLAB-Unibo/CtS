...
class DepthSmooth(_GenericLoss):
    name = "depth_smooth"
    default_target = "depth"

    def __call__(self, image: torch.Tensor, pred: torch.Tensor, **kwargs):
        return inverse_depth_smoothness_loss(pred.to(image.dtype), image)
